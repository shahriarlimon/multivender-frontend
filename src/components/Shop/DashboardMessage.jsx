    ...
    <div className='w-[90%] bg-white m-5 h-[85vh] overflow-y-scroll rounded'>
      ...
      {/* message lists */}
      {
        !open && (<>
          ...
          {
            conversations && conversations?.map((item, idx) => (<MessageLists setActiveStatus={setActiveStatus} online={onlineCheck(item)} setUserData={setUserData} userData={userData} seller={seller._id} setCurrentChat={setCurrentChat} setOpen={setOpen} open={open} data={item} key={idx} index={idx} />))
          }
        </>)
      }
...
const MessageLists = ({ data, index, open, setOpen, setCurrentChat, seller, userData, setUserData, online, setActiveStatus, }) => {
  ...
  useEffect(() => {
    ...
    const userId = data.members.find((user) => user !== seller);
    const getUser = async () => {
      try {
    ...
    getUser();
  }, [seller, data, online, setActiveStatus]);
